...
abstract class WPVQGame {
	...
	*/
	...
	public function add($param, $quizId = NULL) 
	{
		...
		if (!isset($param['name']) || !isset($param['showSharing']) || !isset($param['showCopyright']) || !isset($param['skin']) || !isset($param['askInformations']) || !isset($param['forceToShare']) || !isset($param['randomQuestions']) || !isset($param['isRandomAnswers']) || !isset($param['meta']) || !is_array($param)) {
	...
	private function fetchQuestions($gameplayContext=false, $randomAnswers=false)
	{
		...
		{
			shuffle($questions);
			shuffle($alreadyPlayedQuestions);

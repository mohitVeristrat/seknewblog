	...
	<<?php echo $tag; ?> id="comment-<?php comment_ID(); ?>" <?php comment_class( empty( $args['has_children'] )  ? 'parent' : '', $comment ); ?>>
		<article id="div-comment-<?php comment_ID(); ?>" class="comment-body">
			<footer class="comment-meta">
				...
				<p class="comment-awaiting-moderation"><?php _e( 'Your comment is awaiting moderation.', 'viral' ); ?></p>
				<?php endif; ?>
				<?php edit_comment_link( __( 'Edit', 'viral' ), '<span class="edit-link">', '</span>' ); ?>
			</footer><!-- .comment-meta -->
				...
				<a href="<?php echo esc_url( get_comment_link( $comment, $args ) ); ?>">
					<time datetime="<?php comment_time( 'c' ); ?>">
						<?php
							...
							printf( __( '%1$s at %2$s', 'viral' ), get_comment_date( '', $comment ), get_comment_time() );
						?>
					</time>
				</a>
